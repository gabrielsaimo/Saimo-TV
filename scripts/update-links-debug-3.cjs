...
function normalizeName(name) {
  ...
  normalized = normalized.replace(/[()[\]]/g, ''); // Remove parentheses and brackets
  ...
  for (const file of files) {
    ...
    try {
      ...
      for (const item of enrichedData) {
        ...
        console.log(`[CINEMA] Processing: ${item.name} -> ${normalizedMovieName}`);

        if (m3uEntries.has(normalizedMovieName)) {
          const newUrl = m3uEntries.get(normalizedMovieName);
          console.log(`[CINEMA] Found match for ${item.name}`);
...
async function main() {
  console.log('Iniciando atualizacao de links enriched (debug-3)...');
